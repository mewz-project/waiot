...
#define SECURE_LED_RED_PIN        GPIO_PIN_2
#define SECURE_LED_RED_GPIO_PORT  GPIOG
...
/* USER CODE BEGIN PFP */
static void Secure_LED_EnableVddIO2(void);
...
static void Secure_LED_EnableVddIO2(void)
{
  uint32_t pwr_clock_was_disabled = 0U;

  if (__HAL_RCC_PWR_IS_CLK_DISABLED())
  {
    __HAL_RCC_PWR_CLK_ENABLE();
    pwr_clock_was_disabled = 1U;
  }

  HAL_PWREx_EnableVddIO2();

  if (pwr_clock_was_disabled == 1U)
  {
...
void SECURE_LED_Init(void)
{
  ...
  Secure_LED_EnableVddIO2();
  __HAL_RCC_GPIOG_CLK_ENABLE();

  gpio_init_structure.Pin = SECURE_LED_RED_PIN;
  gpio_init_structure.Mode = GPIO_MODE_OUTPUT_PP;
  gpio_init_structure.Pull = GPIO_NOPULL;
  gpio_init_structure.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
  HAL_GPIO_Init(SECURE_LED_RED_GPIO_PORT, &gpio_init_structure);
...
{
  HAL_GPIO_WritePin(SECURE_LED_RED_GPIO_PORT, SECURE_LED_RED_PIN, GPIO_PIN_SET);
}

void SECURE_LED_Off(void)
{
  HAL_GPIO_WritePin(SECURE_LED_RED_GPIO_PORT, SECURE_LED_RED_PIN, GPIO_PIN_RESET);
}
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_ConfigPinAttributes(GPIOA, GPIO_PIN_10|GPIO_PIN_9, GPIO_PIN_NSEC);
  HAL_GPIO_ConfigPinAttributes(GPIOB, GPIO_PIN_7, GPIO_PIN_NSEC);
  HAL_GPIO_ConfigPinAttributes(GPIOC, GPIO_PIN_13|GPIO_PIN_7, GPIO_PIN_NSEC);
